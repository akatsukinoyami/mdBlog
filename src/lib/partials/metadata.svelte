<script lang="ts">
	...
	import { Counts, type Entity } from '$lib/types';
	...
	async function action(postId: UUID | undefined, type: keyof Counts) {
		if (!postId) return;
		const variables =
			sessionStorage.getItem(`${postId}/${type}`) === 'true'
				? { post_id: postId }
				: { post_id: postId, type };
		...
		fetch(url(['api', 'post'], variables))
			.then((res) => res.json())
			.then((json) => {
				metadata = json?.message === 'Internal Error' ? undefined : json;
			})
